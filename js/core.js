...
boards.forEach(function (meta, board_id) {
  ...
  console.log(
    `Found board ${board_id} - "${meta.title}", revision ${meta.current}, history [${hist}], backup ${JSON.stringify(meta.backupStatus)}`,
  );
});
...
console.log(
  `Font:      [${conf.fontName}], size [${conf.fontSize || "-"}], line-height [${conf.lineHeight || "-"}]`,
);
